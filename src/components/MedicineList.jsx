...
export default function MedicineList({ 
  ...
  const stats = useMemo(() => {
    ...
    const pending = entries.filter(e => new Date(e.date) <= new Date() && !e.taken).length;
    return { total, taken, pending, percentage: total > 0 ? Math.round((taken / total) * 100) : 0 };
  }, [entries]);

  const formatDateRange = () => {
    if (!medication.startDate) return "Sin fecha definida";
    const start = new Date(medication.startDate).toLocaleDateString('es-ES', { day:'2-digit', month:'short', year:'numeric' });
    if (medication.lifetime) return `Desde ${start} (de por vida)`;
    if (!medication.endDate)  return `Desde ${start}`;
    const end = new Date(medication.endDate).toLocaleDateString('es-ES', { day:'2-digit', month:'short', year:'numeric' });
    return `${start} - ${end}`;
  };
    ...
    <section className={`w-full max-w-full mb-8 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden ${!isActive ? "opacity-75" : ""}`}>
      ...
      <header className={`${isActive ? "bg-gradient-to-r from-teal-50 to-teal-100" : "bg-gradient-to-r from-gray-50 to-gray-100"} p-5 sm:p-6 border-b ${isActive ? "border-teal-200" : "border-gray-200"}`}>
        <div className="flex flex-col gap-3">
          ...
          {/* Stats */}
          <div className="md:self-end md:w-[180px]">
            <div className={`rounded-lg p-3 shadow-sm border ${isActive ? "bg-white border-gray-200" : "bg-gray-50 border-gray-200"}`}>
              <div className={`text-xl sm:text-2xl font-bold ${isActive ? "text-teal-900" : "text-gray-600"}`}>{stats.percentage}%</div>
              <div className={`${isActive ? "text-teal-600" : "text-gray-500"} text-xs`}>Adherencia</div>
              <div className="text-xs text-gray-500 mt-1">{stats.taken}/{stats.total} tomadas</div>
              {stats.pending > 0 && isActive && (
                <div className="text-xs text-red-600 mt-1">{stats.pending} pendientes</div>
              )}
            </div>
          </div>
        </div>
